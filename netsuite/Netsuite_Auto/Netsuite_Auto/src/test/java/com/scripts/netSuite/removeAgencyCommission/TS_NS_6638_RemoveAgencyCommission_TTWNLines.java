...
public class TS_NS_6638_RemoveAgencyCommission_TTWNLines extends BaseReport{


	private BaseTest basetest;
	public static Excel_Reader excelReader;
	public static int i=6638;
	...
	//=============>
	@Test
	public void RemoveAgencyCommissionTTWNLines() throws InterruptedException, IOException, AWTException {
		...
		//Remove Agency Case Page validations
		...
		details[0].toString(); agencyCommissionInUI = details[1].toString();
		  ...
		  if(pdfData[10].contains(XLTestData.get("invoiceNumber"))) { 
			  basetest.test.log(Status.PASS, "Invoice Number is Published in PDF document");
			  } 
		  else{
		  basetest.test.log(Status.FAIL, "Invoice Number is not  Published in PDF document");
		  } 

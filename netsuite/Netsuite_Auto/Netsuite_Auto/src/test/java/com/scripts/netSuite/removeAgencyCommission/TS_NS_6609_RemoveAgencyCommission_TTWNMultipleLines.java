...
public class TS_NS_6609_RemoveAgencyCommission_TTWNMultipleLines extends BaseReport {

	private BaseTest basetest;
	public static Excel_Reader excelReader;
	public static int i = 6609;
	...
	// =============>
	@Test
	public void RemoveAgencyCommissionTTWNMultipleLines() throws InterruptedException, IOException, AWTException {
		...
		// Remove Agency commison Case Page validations
		...
		details[0].toString(); agencyCommissionInUI = details[1].toString();
		  ...
		  "Gross Amount(<span style='font-weight:bold;color:blue'>"
		  +grossAmountInUI+"</span>) in UI and in PDF document are equal"); } else{
		  basetest.test.log(Status.FAIL,
		  "Gross Amount("+grossAmountInUI+") in UI and in PDF document are not equal");
		  } if(pdfData[9].contains(agencyCommissionInUI)) {
		  basetest.test.log(Status.PASS,"Agency Commission("
		  +agencyCommissionInUI+") in UI and in PDF document are equal"); } else {
		  basetest.test.log(Status.FAIL, "Agency Commission("
		  +agencyCommissionInUI+") in UI and in PDF document are not equal"); }
